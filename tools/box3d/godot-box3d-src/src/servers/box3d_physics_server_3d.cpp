...
void Box3DPhysicsServer3D::_joint_make_cone_twist(const RID& p_joint, const RID& p_body_a, const Transform3D& p_local_ref_a, const RID& p_body_b, const Transform3D& p_local_ref_b) {
	...
	// ConeTwistJoint3D's _local_ref_a/b are rotation-bearing Transforms (the cone's local
	// Z axis is the swing axis; cone + twist limits are expressed around it). We forward
	// them as-is to Box3D whose SphericalJoint's cone axis is bodyA's local Z.
