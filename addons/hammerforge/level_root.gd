...
# FIX LAG EDITOR: throttle del _process en el editor. process_thread_queue()
# y io_visualizer.process() no necesitan 60 Hz; 10 Hz basta y alivia el pico
# de trabajo al minimizar/maximizar la ventana del editor.
var _process_acc: float = 0.0
const _PROCESS_THROTTLE_SEC := 0.1
...
func _process(delta: float) -> void:
	if not Engine.is_editor_hint():
		return
	# FIX LAG EDITOR: throttle a 10 Hz. El I/O y los previews de brush no
	# necesitan 60 fps en el editor; reducir a 10 Hz elimina el pico de
	# trabajo al cambiar el foco de la ventana (minimize/maximize).
	_process_acc += delta
	if _process_acc < _PROCESS_THROTTLE_SEC:
		return
	_process_acc = 0.0
	...
	if subtract_preview and subtract_preview.is_enabled():
		subtract_preview.process(_PROCESS_THROTTLE_SEC)
...
func _raycast(camera: Camera3D, mouse_pos: Vector2) -> Dictionary:
	...
	if not best_hit.is_empty():
		return best_hit
	var plane = Plane(Vector3.UP, grid_plane_origin.y)
	var denom = plane.normal.dot(to - from)
	if abs(denom) > 0.0001:
		var t = plane.distance_to(from) / denom
		if t >= 0.0:
			return {"position": from + ray_dir * t}
	# Fallback: use grid system intersection with the current grid plane
	var grid_hit = _intersect_axis_plane(camera, mouse_pos, grid_plane_axis, grid_plane_origin)
	if grid_hit is Vector3:
		return {"position": grid_hit}
	# Final fallback: project along ray to a reasonable distance
	return {"position": from + ray_dir * 100.0}
