...
# FIX LAG EDITOR: throttle del _process a 4 Hz en el editor. La búsqueda
# de LevelRoot y la reconexión de señales no necesitan correr a 60 fps.
var _dock_process_acc: float = 0.0
const _DOCK_PROCESS_THROTTLE_SEC := 0.25
...
func _process(_delta):
	# FIX LAG EDITOR: la búsqueda de edited_scene_root / LevelRoot y la
	# lógica de (re)conexión de señales se hacían cada frame en el editor.
	# Throttle a 4 Hz (cada 0.25s) — la escena no cambia tan rápido y el
	# resto de UI updates ya estaba protegido por _hints_dirty / frame counter.
	_dock_process_acc += _delta
	if _dock_process_acc < _DOCK_PROCESS_THROTTLE_SEC:
		return
	_dock_process_acc = 0.0
...
func get_shape() -> int:
	print("[HF DEBUG] get_shape() called, active_shape=", active_shape, " shape_select.selected=", shape_select.selected if shape_select else -1)
	return active_shape
...
func _on_shape_selected(index: int) -> void:
	...
	print("[HF DEBUG] _on_shape_selected called: index=", index, " shape_value=", shape_value)
	_set_active_shape(shape_value)


func _set_active_shape(shape_value: int) -> void:
	print("[HF DEBUG] _set_active_shape: ", active_shape, " -> ", shape_value)
