...
func _update_context_toolbar_state(root: Node, tool_id: int) -> void:
	...
	if root and root.prefab_system and not hf_selection.is_empty():
		var first_node: Node = hf_selection[0]
		if not first_node is Node3D:
			first_node = null
		var pfb_iid: String = str(first_node.get_meta("hf_prefab_instance", "")) if first_node else ""
		if pfb_iid != "":
...
func _handle_draw_mouse(
	...
	# DEBUG: Print shape being used for drawing
	print("[HF DEBUG] Drawing with shape: ", shape, " (0=BOX, 1=CYL, 2=SPH, 3=CONE), size: ", size)
	if event.pressed:
		# If we're in height mode, finalize the brush immediately on click
		if root.input_state and root.input_state.is_drag_height():
			print("[HF DEBUG] Finalizing brush in height mode")
...
func _on_context_toolbar_action(action: String, args: Array) -> void:
	# Handle shape changes FIRST — these don't require a LevelRoot
	match action:
		"shape_box":
			print("[HF DEBUG] Toolbar: shape_box selected")
			if dock and dock.shape_select:
				dock.shape_select.select(0)
				dock._on_shape_selected(0)
				print("[HF DEBUG] Dock shape set to BOX (0)")
			return
		"shape_cylinder":
			print("[HF DEBUG] Toolbar: shape_cylinder selected")
			if dock and dock.shape_select:
				dock.shape_select.select(1)
				dock._on_shape_selected(1)
				print("[HF DEBUG] Dock shape set to CYLINDER (1)")
			return
		"shape_sphere":
			print("[HF DEBUG] Toolbar: shape_sphere selected")
			if dock and dock.shape_select:
				dock.shape_select.select(2)
				dock._on_shape_selected(2)
				print("[HF DEBUG] Dock shape set to SPHERE (2)")
			return
		"shape_cone":
			print("[HF DEBUG] Toolbar: shape_cone selected")
			if dock and dock.shape_select:
				dock.shape_select.select(3)
				dock._on_shape_selected(3)
				print("[HF DEBUG] Dock shape set to CONE (3)")
			return
