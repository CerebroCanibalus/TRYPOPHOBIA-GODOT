...
func begin_drag(
	...
	if not input_state.is_idle():
		print("[HF DEBUG] begin_drag: input_state not idle (mode=", input_state.mode, "), forcing reset")
		cancel_drag()
		# Try again after reset
		if not input_state.is_idle():
			print("[HF DEBUG] begin_drag FAILED: still not idle after reset")
			return false
	var hit = root._raycast(camera, mouse_pos)
	if not hit:
		print("[HF DEBUG] begin_drag FAILED: raycast hit nothing at mouse_pos=", mouse_pos)
		return false
	var snapped_origin = root._snap_point(hit.position)
	var height = root.grid_snap if root.grid_snap > 0.0 else size.y
	print("[HF DEBUG] begin_drag SUCCESS: origin=", snapped_origin, " shape=", shape, " height=", height)
