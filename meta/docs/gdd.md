...
1. **Paranoia** — Nunca sabes quién es el enemigo. Cada decisión de confianza tiene peso real. El Asimilado se ve idéntico a un misionero sano; la duda es constante y estructural.

2. **Agencia Continua** — Ningún jugador está nunca "fuera de la partida". Misionero, Asimilado y Director son roles activos con poder propio. Morir es un cambio de rol, no un tiempo muerto.

3. **Identidad Cultural** — El mundo, los personajes y el lore emanan de Gran Iberia. El setting político-cultural hispanista e iberófono no es cosmético: es la voz del juego.

4. **Horror Emergente** — El miedo más efectivo surge del caos entre jugadores, no de sustos programados. La comedia y el terror coexisten como consecuencia natural de la interacción humana.

**Prioridad ante conflictos:** Paranoia > Agencia Continua > Horror Emergente > Identidad Cultural.

### Core Gameplay Loop

**Loop principal (~45 minutos):**

```
LOBBY
  └─► Viaje a la misión
        └─► Exploración de la ciudad
              ├─► Recoger objetos y herramientas
              ├─► Buscar acceso a la Colmena
              └─► [ASIMILADO: identificar y eliminar misioneros]
                    └─► Destruir la Colmena
                          ├─► [Victoria] Misioneros — Colmena destruida
                          └─► [Victoria] Asimilado — todos los sanos eliminados
```

**Roles en el loop:**

- **Misionero:** Explora, recoge objetos, coordina con el equipo, desconfía, busca y destruye la Colmena.
- **Asimilado:** Se mezcla con los misioneros, elige el momento de transformarse, elimina objetivos sin revelar su identidad más de lo necesario.
- **Director** *(jugador muerto en cooldown)*: Espectea a los misioneros sanos e interactúa con el entorno — puertas, luces, maquinaria — para apoyar o dificultar según su lealtad.

**Duración típica del loop:** ~45 minutos por partida completa.

**Variación entre iteraciones:** Personaje elegido, objeto inicial único, quién es el Asimilado, disposición del mapa y orden de descubrimiento de la Colmena.

### Win/Loss Conditions

#### Victoria — Misioneros
La Colmena es destruida. No se requiere volver a la nave (la extracción es un objetivo secundario opcional). Ganan aunque hayan perdido jugadores durante la misión.

#### Victoria — Asimilado
Todos los jugadores misioneros sanos son eliminados antes de que la Colmena sea destruida.

#### Sin límite de tiempo
Las misiones no tienen cronómetro. La presión proviene del Asimilado y de la dificultad de acceder a la Colmena, no de un temporizador artificial.

#### Derrota y Consecuencias
No hay penalizaciones persistentes entre partidas — cada misión es independiente. *A considerar en fases futuras:* consecuencias opcionales de derrota (pérdida de objetos desbloqueables, variación narrativa) sin afectar el acceso al juego base.
...
#### 1. Movimiento FPS con Stamina
*Pilar: Agencia Continua*
Caminar, correr (sprint), agacharse y saltar con sistema de stamina. El sprint y el salto consumen stamina; si se agota, hay cooldown de recuperación. La stamina añade decisiones tácticas de movilidad.

**Wall Jump:** Todos los jugadores pueden hacer wall jump saltando junto a una pared mientras están en el aire. Amplía la movilidad vertical y permite acceder a zonas elevadas.

#### 2. Sistema de Inventario (4 objetos)
*Pilar: Agencia Continua*
En modo normal, cada jugador porta hasta 4 objetos ciclables con Scroll. Los objetos son de dos tipos:
- **Objeto único del personaje** — cada uno de los 12 personajes comienza la misión con un objeto exclusivo que define parte de su rol estratégico.
- **Objetos del entorno** — objetos especiales interactuables encontrados durante la exploración (armas, consumibles, herramientas). No son rigidbodies: tienen lógica propia de uso.

#### 3. Interacción con Objetos y Entorno
*Pilar: Agencia Continua*
`E` interactúa con el mundo: usar el objeto activo del inventario, activar mecanismos, acceder a la Colmena.
`Click izquierdo` agarra RigidBodies del entorno.
`Click derecho` empuja objetos y entidades con el rayo de cámara.

#### 4. Transformación del Asimilado
*Pilar: Paranoia*
Manteniendo `F`, el Asimilado alterna entre su forma humana (indistinguible del resto) y su forma monstruosa (capaz de matar). La transformación es voluntaria y costosa — revelar la forma monstruosa expone al Asimilado a ser identificado.

#### 5. Habilidades del Asimilado (forma monstruosa)
*Pilar: Paranoia + Horror Emergente*
En forma monstruosa, el Asimilado tiene acceso a 3 habilidades ciclables con Scroll y activables con `Q`. Las habilidades se definen por personaje/monstruo en el diseño de personajes.

#### 6. Control de Entorno — Director
*Pilar: Agencia Continua*
Los jugadores muertos juegan como Directores mientras esperan su reaparición. Pueden interactuar con elementos del entorno: puertas, luces, maquinaria. No son fantasmas pasivos — tienen influencia real sobre el espacio de juego.

#### 7. Destruir la Colmena
*Pilar: Agencia Continua + Horror Emergente*
Objetivo principal de los misioneros. Requiere exploración previa para encontrar acceso. El método exacto de destrucción se define por mapa.

### Interacciones entre Mecánicas

- El **inventario** y el **wall jump** dan herramientas de exploración que el Asimilado en forma humana también puede usar — mantiene la igualdad visual entre roles.
- El **Scroll** es contextual: cicla objetos en modo normal y habilidades en forma monstruosa, sin conflicto de input.
- El **Director** puede usar el control de entorno para ayudar al Asimilado (apagar luces durante un ataque) o sabotear (abrir una puerta de escape a misioneros). Su lealtad es implícita.

### Progresión de Mecánicas

No hay progresión dentro de la partida. Los personajes entran con sus stats y objeto único definitivos. La variedad entre partidas viene del personaje elegido y los objetos encontrados en el entorno.

### Controls and Input

**Esquema de Controles (PC — Teclado + Ratón)**
...
| Agarrar objeto | Click izquierdo | Modo normal |
| Empujar | Click derecho | Modo normal |
| Ciclar habilidades | Scroll | Asimilado transformado |
| Activar habilidad | Q | Asimilado transformado |
| Transformar (hold) | F | Solo Asimilado |

**Input Feel:** Movimiento FPS responsivo con lerp suave. La transformación con `F` mantenido añade tensión táctil — no es instantánea, requiere compromiso.

**Accesibilidad:** *A definir en fases posteriores.* Mínimo recomendado: rebinding completo de teclas, sensibilidad de ratón ajustable.
